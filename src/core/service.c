...
static int service_verify(Service *s) {
         ...
         * Hence, if delegation is on, mandate a subcgroup per configuration.
        ...
        if ((s->cgroup_context.delegate && !s->cgroup_context.delegate_subcgroup) &&
            (s->exec_command[SERVICE_EXEC_START_POST] ||
             ...
             s->exec_command[SERVICE_EXEC_STOP_POST]))
                return log_unit_error_errno(UNIT(s), SYNTHETIC_ERRNO(ENOEXEC), "Service with delegated cgroup subtree defines control commands but no DelegateSubControlGroup= is specified.");
...
static void service_run_next_control(Service *s) {
        ...
        r = service_spawn(s,
                          ...
                          (IN_SET(s->control_command_id, SERVICE_EXEC_CONDITION, SERVICE_EXEC_START_PRE) ? EXEC_NO_SUBCGROUP : 0),
                          &s->control_pid);
        if (r < 0)
                goto fail;
