...
bool unit_instance_is_empty(UnitInstanceArg i) {
...
bool unit_instance_is_valid(UnitInstanceArg i) {
        ...
        return !unit_instance_is_empty(i);
}
...
int unit_name_build_from_type(const char *prefix, UnitInstanceArg instance, UnitType type, char **ret) {
        ...
        if (unit_instance_is_empty(instance))
                s = strjoin(prefix, ".", ut);
        else if (!unit_instance_is_valid(instance))
                return -EINVAL;
        ...
        if (!unit_name_is_valid(s, !unit_instance_is_empty(instance) ? UNIT_NAME_INSTANCE : UNIT_NAME_PLAIN))
                return -EINVAL;
