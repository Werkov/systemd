...
cat >/run/systemd/system/testservice-84-foo#.service <<EOF
[Service]
...
logger "MARK start"
systemctl start testservice-84-foo.service
# check one gen exists
active_inst="$(systemctl show -p Following --value testservice-84-foo.service)"
[[ "$active_inst" =~ testservice-84-foo#[^.]*.service ]]
systemctl is-active "$active_inst"

logger "MARK restart"
systemctl restart testservice-84-foo.service
# check two gens exist
active_inst2="$(systemctl show -p Following --value testservice-84-foo.service)"
...
active_inst3="$(systemctl show -p Following --value testservice-84-foo.service)"
[[ "$active_inst3" != "$active_inst" ]]
...
logger "MARK stop"
systemctl stop testservice-84-foo.service
sleep 1
# check no gen exists
! systemctl is-active "$active_inst"
! systemctl is-active "$active_inst2"
