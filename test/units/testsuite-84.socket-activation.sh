...
cat >/run/systemd/system/testsuite-84-bar.socket <<EOF
[Socket]
ListenStream=/run/testsuite-84-bar.sock
EOF

cat >/run/systemd/system/testsuite-84-bar#.service <<EOF
[Service]
...
systemctl start testsuite-84-bar.socket

# Run 10 clients on the background
for i in {1..10}; do
    socat -t2 GOPEN:"/tmp/client-$i.log" UNIX-CONNECT:/run/testsuite-84-bar.sock &
done
...
systemctl restart testsuite-84-bar.service

# Connect one more client now after service is restarted
socat -t2 GOPEN:/tmp/client-11.log UNIX-CONNECT:/run/testsuite-84-bar.sock &
...
# Stop the test units
systemctl stop testsuite-84-bar.socket
systemctl stop testsuite-84-bar.service
